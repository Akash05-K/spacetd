...
*   **uvicorn:** ASGI server used to run the FastAPI service. Install with:
    ```cmd
    pip install "uvicorn[standard]"
    ```
*   **streamlit:** (for dashboards) Install with:
    ```cmd
...
1.  **Clone the Repository:**
    ```bash
    git clone https://github.com/Akash05-K/CryptoBot.git
    cd CryptoBot
    ```
...
4.  **Set up Environment Variables:**
    *   Copy `.env.example` to `.env` (choose the command for your shell):
        - On Unix/macOS or Git Bash:
        ```bash
        cp .env.example .env
        ```
        - On Windows `cmd.exe`:
...
### Running the API & Dashboard locally (without Docker)

If you prefer to run services locally without Docker, start each service in a separate terminal while your virtualenv is activated.
...
MLflow (if installed and configured): refer to `pyproject.toml`/`requirements.txt` for the command to run the tracking server (often `mlflow ui --port 5000`).
